...
const InactiveUsers = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const activeUsers = response.data.user.filter(user => user.status === "inactive");
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      // If user was set to active, remove from inactive users list
      if (formData.status === "active") {
        setUsers(users.filter(user => user._id !== editUsersId));
      } else {
    ...
    <div className="flex flex-col md:flex-row bg-black text-white">
      <main className="flex-1 mt-6 lg:p-6">
        <header className="mb-6 flex justify-between items-center px-4">
          <div>
            <h2 className="text-2xl font-bold text-[#0f6dd3]">Inactive Users</h2>
            <p className="text-gray-400 text-sm mt-1">Showing {users.length} Inactive user accounts</p>
          </div>
          <div className="flex space-x-3">
            <Link to="/activeUsers" className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md transition">
              View Active Users
            </Link>
          </div>
        </header>
              ...
              {users.map((user) => (
                <tr key={user._id} className="border-b border-gray-700">
                  ...
                  <td className="py-3 px-4 flex items-center space-x-2">
                    ...
                    <button className="bg-amber-500 text-white p-2 rounded-md hover:bg-amber-600"
