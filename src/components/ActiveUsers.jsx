...
const ActiveUsers = () => {
    ...
    const fetchUsers = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          const activeUsers = response.data.user.filter(user => user.status === "active");
  ...
  const handleUpdateUser = async (e) => {
    ...
    try {
      ...
      // If user was set to inactive, remove from active users list
    ...
    <div className="flex flex-col md:flex-row bg-black text-white">
      <main className="flex-1 mt-6 lg:p-6">
        <header className="mb-6 flex justify-between items-center px-4">
          <div>
            <h2 className="text-2xl font-bold text-[#0f6dd3]">Active Users</h2>
            <p className="text-gray-400 text-sm mt-1">Showing {users.length} active user accounts</p>
          </div>
          <div className="flex space-x-3">
            <Link to="/inactive-users" className="bg-gray-700 hover:bg-gray-600 text-white py-2 px-4 rounded-md transition">
              View Inactive Users
            </Link>
          </div>
        </header>
